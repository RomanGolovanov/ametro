...
public class OnlineMaps extends Activity implements ICatalogStorageListener {
		...
		if(mMode != MODE_LIST){
			if(mOnlineDownload){
				...
				if(mOnline == null){
					...
					if(mLocal!=null && mOnline!=null && mOnline.getMaps().size()>0){
						setListView();
					}else{
						setEmptyView();
					}
	...
	private void setEmptyView() {
		...
		((TextView)findViewById(R.id.maps_message)).setText(R.string.msg_no_maps_in_online);
		mMode = MODE_EMPTY;
	} 
	
	private void setListView() {
		setContentView(R.layout.browse_catalog_list_item);
		mCatalogDifferences = Catalog.diff(mLocal, mOnline, Catalog.MODE_RIGHT_JOIN);
